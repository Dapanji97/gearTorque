...
void main() {
  ...
  while(choice != 4) {
    ...
    //printf("[] Create new vehicle profile.\n");
    //printf("[]Display list of vehicles on file.\n");
    printf("[3] Save current vehicle data.\n");
    printf("[4] Exit.\n");
    scanf("%d", &choice);
...
//A function to be called when user wishes to save data into a .txt file.
void dataWrite() { //Shall receive pointer of structure for different vehicles.
  char choice;
  char fName[20];

  printf("Vechicle name: ");
  scanf("%s", pTrain.vName);
  ...
  fprintf(fp, "== PEAK TORQUE ===============================================|\n");
  fprintf(fp, "      SCENARIO 1                                                 \n");
  fprintf(fp, "==============================================================|\n");
  fprintf(fp, "|G1|   %.3lf Nm   %.3lf N   \n", Output.wTorque[1], Output.wForce[1]);
  fprintf(fp, "|G2|   %.3lf Nm   %.3lf N   \n", Output.wTorque[2], Output.wForce[2]);
  fprintf(fp, "|G3|   %.3lf Nm   %.3lf N   \n", Output.wTorque[3], Output.wForce[3]);
  fprintf(fp, "|G4|   %.3lf Nm   %.3lf N   \n", Output.wTorque[4], Output.wForce[4]);
  fprintf(fp, "|G5|   %.3lf Nm   %.3lf N   \n", Output.wTorque[5], Output.wForce[5]);
  fprintf(fp, "==============================================================|\n");
}
